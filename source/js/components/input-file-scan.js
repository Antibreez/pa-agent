(function () {
  ...
  const fileInputItem = scanUpload.querySelectorAll(".input-file");
  ...
  function isFilled() {
    ...
    fileInputItem.forEach((item) => {
      const file = item.querySelector(".file-load");
  ...
  function readUrl(input) {
    ...
    const bar = fileDropArea.parentNode.querySelector(
      ".file-load__progress-current"
    );
    const status = fileDropArea.parentNode.querySelector(
      ".file-load__status span"
    );
    ...
    if (input.files && input.files[0]) {
      ...
      reader.onload = function (e) {
        bar.style.width = "100%";
        fileDropArea.nextElementSibling.querySelector(
          ".file-load__name"
        ...
        isFilled() && submit.removeAttribute("disabled");
